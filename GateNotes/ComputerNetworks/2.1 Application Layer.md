...
- **Does the Application layer protocols check for errors in the message?**
	...
	- These protocols assume that the network is error free
		- This means that the message sent form the other host is intact
...
- **HTTP Request syntax**
	- Request line:
		...
		- HTTP version
	- Request headers
		- Has various fields like client program, accepted language
...
- **HTTP pipelining**
	- #TODO 
	- Send multiple files over the same TCP connection without waiting for the response
...
- **What happens when user requests file?**
	- A FTP client establishes a TCP connection with the FTP server
...
- **Where is it used?**
	...
	- User sends a mail to another user using the client application
		...
		- The recipient uses their user agent to pool their mail box
...
- **POP(Post office protocol) and IMAP(Internet Mail Access Protocol)**
	...
	- POP: Stateful protocol used to download the message from the server
	- IMAP: 
...
- **DNS Server architecture**
	- Consists of a three level hierarchy
		...
		- Top level Domain(TLD) DNS:
			- DNS resolution request is received, this server which sends the address of the Autoritative server
			- Responsible for top level domain such as .org .edu .com
...
- **Does the DNS Architecture always have three levels?**
	...
	- When A DNS resolution is required for this name
		- There will be 4 request response pairs for local DNS to DNS servers
...
- **What happens when a host requires the IP of the domain \www.something.edu?**
	...
	- The local DNS forwards this request to the root DNS server 
		- Root DNS server notes the .edu and sends the IP of a TLD DNS server to the local DNS server
	- The local DNS server sends the domain name to the TLD server
		- The TLD DNS server sends the IP address of an authoritative server to the local DNS server
...
- **DNS Caching**:
	...
	- Each domain name mapping is invalidated after a period off time
